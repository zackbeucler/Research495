...
class Workstation():
    def __init__(self, name,cx,cy,radius, color, outlet_id):
    ...
    def controlLights(self, light_ID, option):
        option = option.lower()
        url = ''
        if (option == 'on'):
            url = 'http://136.244.192.61/apps/api/3/devices/'+str(light_ID)+'/on?access_token=e672d44a-d483-4f3d-91f6-1e67404b218e'
        elif (option == 'off'):
            url = 'http://136.244.192.61/apps/api/3/devices/'+str(light_ID)+'/off?access_token=e672d44a-d483-4f3d-91f6-1e67404b218e'
        else:
            print('Wrong option inputted')
            return    
        result = requests.get(url)
        if result.status_code != 200:
            print('error controlling light: '+str(light_ID)+' status code: '+str(result.status_code))
    ...
    def clean(self): # this method needs a thread
        ...
        self.controlLights(self.outlet_id, 'on')
        time.sleep(10)
        self.controlLights(self.outlet_id, 'off')
